...
function animasikan(){
    ...
    background.update(); 
    if(player.position.y > semak1.position.y + 10){
        semak1.update();
    }
    if(player.position.y > semak2.position.y + 10){
        semak2.update();
    }
    if(player.position.y > aple.position.y){
        aple.update();
    }
    ...
    //
    // if(!mapLimit(player)){
        // console.log(background.angkageserMap, background.position)
    ...
    // collider
    if(player.onattack && player.position.x > semak1.position.x -(128/2) && player.position.x < semak1.position.x+(128/2)
        && player.position.y > semak1.position.y-(128/2) && player.position.y < semak1.position.y+(128/2)){
        semak1.position.x = -100;
        console.log('makan g');
        player.onattack = false;
    }
    if(player.onattack && player.position.x > aple.position.x -(128/2) && player.position.x < aple.position.x+(128/2)
        && player.position.y > aple.position.y-(128/2) && player.position.y < aple.position.y+(128/2)){
        aple.position.x = -100;
        console.log('makan g');
        player.onattack = false;
        getItem = true;
